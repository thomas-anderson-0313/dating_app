        ...
        public void OnSelection(MaterialDialog p0, View p1, int itemId, ICharSequence itemString)
        {
            try
            {
                switch (TypeDialog)
                {
                    ...
                    case "FromHeight":
                        FromHeight = ListUtils.SettingsSiteList?.Height?[itemId]?.Keys.FirstOrDefault() ?? UserDetails.FilterOptionFromHeight;
                        EdtFromHeight.Text = itemString.ToString();
                        break;
                    case "ToHeight":
                        ToHeight = ListUtils.SettingsSiteList?.Height?[itemId]?.Keys.FirstOrDefault() ?? UserDetails.FilterOptionToHeight;
