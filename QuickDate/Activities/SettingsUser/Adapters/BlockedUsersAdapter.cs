   ...
   public class BlockedUsersAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
        ...
        public void Initialize(BlockedUsersAdapterViewHolder holder, Block users)
        {
            try
            {
                GlideImageLoader.LoadImage(ActivityContext, users.Data.Avater.PurpleUri, holder.ImageUser, ImageStyle.CircleCrop, ImagePlaceholders.Drawable);
        ...
        public IList GetPreloadItems(int p0)
        {
            try
            {
                ...
                if (item.Data?.Avater.PurpleUri != "")
                {
                    d.Add(item.Data?.Avater.PurpleUri);
                    return d;
                }
