        ...
        public void OnSelection(MaterialDialog p0, View p1, int itemId, ICharSequence itemString)
        {
            try
            {
                switch (TypeDialog)
                {
                    case "Ethnicity":
                    {
                        var ethnicityArray = ListUtils.SettingsSiteList?.Ethnicity?.FirstOrDefault(a => a.ContainsValue(itemString.ToString()))?.Keys.FirstOrDefault();
                    ...
                    case "Body":
                    {
                        var bodyArray = ListUtils.SettingsSiteList?.Body?.FirstOrDefault(a => a.ContainsValue(itemString.ToString()))?.Keys.FirstOrDefault();
                    ...
                    case "HairColor":
                    {
                        var hairColorArray = ListUtils.SettingsSiteList?.HairColor?.FirstOrDefault(a => a.ContainsValue(itemString.ToString()))?.Keys.FirstOrDefault();
                        IdHairColor = int.Parse(hairColorArray ?? "1");
                        EdtHairColor.Text = itemString.ToString();
                        break;
                    }
                    case "Height":
                        IdHeight = ListUtils.SettingsSiteList?.Height?[itemId]?.Keys.FirstOrDefault();
