    ...
    public class NotificationsAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
            ...
            try
            {
                if (viewHolder is NotificationsAdapterViewHolder holder)
                {
                    ...
                    if (item != null)
                    {
                        ...
                        GlideImageLoader.LoadImage(ActivityContext,item.Notifier.Avater.PurpleUri, holder.ImageUser, ImageStyle.CircleCrop, ImagePlaceholders.Drawable);
        ...
        public IList GetPreloadItems(int p0)
        {
            try
            {
                ...
                if (item.Notifier.Avater.PurpleUri != "")
                {
                    d.Add(item.Notifier.Avater.PurpleUri);
                    return d;
                }
